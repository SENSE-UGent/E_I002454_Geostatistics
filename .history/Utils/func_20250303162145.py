...
def inverse_distance_weighting2D(data, grid_points, power, max_points, min_points, search_radius):
    # Iterate over each grid point
    for row in grid_points.iterrows():
        # Calculate distances from data points to the current grid point
        distances = np.sqrt((data.x - row[1]['x'])**2 + (data.y - row[1]['y'])**2)
        ...
        if data_dist['dist'].min() == 0:
            ...
            if len(data_dist.loc[data_dist['dist'] < search_radius]) < min_points:
                ...
                print('The function has finished. The results are in the grid_points DataFrame.')
