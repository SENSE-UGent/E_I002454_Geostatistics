...
def inverse_distance_weighting3D(data, grid_points, data_x_col, data_y_col, grid_x_col, grid_y_col, power, max_points, min_points, search_radii):
    data = data.dropna(subset='o')
    
    if 'z' in data.columns and 'z' in grid_points.columns:
        data_full = data[[data_x_col, data_y_col, 'z', 'o']].values
        data_coords = data[[data_x_col, data_y_col, 'z']].values
        grid_coords = grid_points[[grid_x_col, grid_y_col, 'z']].values
        is_3d = True
    else:
        data_full = data[[data_x_col, data_y_col, 'o']].values
