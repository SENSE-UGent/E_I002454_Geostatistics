...
def is_within_ellipsoid_rework_2(points, center, search_radii):
    ...
    # Compute squared distances without normalization
    x_dist = (x - x_c)**2
    y_dist = (y - y_c)**2
    z_dist = (z - z_c)**2
    ...
    # Create a boolean mask for points within the ellipsoid
    ...
    # print(ellipsoid_distances)

    # # Create an array of distances for all points within the ellipsoid
    euclidean_distances = np.sqrt(squared_distances[within_ellipsoid_mask])

    # Sort points within the ellipsoid by distance from the center
    ...
    # sorted_distances = np.argsort(closest_distances)
...
def inverse_distance_weighting3D_20_beta(data, grid_points, power, max_points, min_points, search_radii):
    ...
    for i in range(len(grid_coords)):
        ...
        if n_near_data < min_points:
            ...
            # kdtree = KDTree(near_data)

            # # max_total_points = n_near_data

            # distances, indices = kdtree.query(grid_point, 
            #                                 #   k=max_total_points
            #                                   )
