...
def inverse_distance_weighting(data=np.nan, grid_points=np.nan, data_x_col=np.nan, data_y_col=np.nan, data_z_col=np.nan,
    ...
    grid_points['idw' + str(power)] = p_list
    grid_points['idw' + str(power)].replace('--', np.nan, inplace=True)
    print('The function has finished. The predictions are in the grid_points DataFrame in the column 'idw' + str(power)')
